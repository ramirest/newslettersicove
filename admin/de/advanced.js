...
StyleCombo.prototype.Redraw = function( parentToolbar ){
	...
	for (i=0;i<this.Items.length;i++){
		var a = window.activeEditor.cssText[i];
		if (a.charAt(a.length-1)!=';') a=a+';';
		a = a.replace(/url\([^)]*\)/gi,'');
		a = a.replace(/position:[^;]*;/gi,'');
		a = a.replace(/(top|right|bottom|left):[^;]*;/gi,'');
		a = a.replace(/(height|width):[^;]*;/gi,'');
		a = a.replace(/(border|padding):[^;]*;/gi,'');
		var add_style = '<span unselectable="on" '+((combostyle_flag)?'style="'+a+'">':">")+this.Items[i] + '</span>';
		opt=opt+'<tr><td unselectable="on" class="de_Combo_head" onclick=\'javascript:ComboOnChange("'+this.Command+'","'+this.Items[i]+'")\' onmouseout=\'javascript:this.className="de_combolist_item"\' onmouseover=\'javascript:this.className="de_combolist_item_highlight"\'><span style="padding-left:3px;" unselectable="on"> '+add_style+'</span></td></tr>';
	}
	opt+='<tr><td><hr/ noshade size=1></td></tr>';
	opt+='<tr><td unselectable="on" class="de_Combo_head" onclick=\'javascript:ComboOnChange("'+this.Command+'","")\' onmouseout=\'javascript:this.className="de_combolist_item"\' onmouseover=\'javascript:this.className="de_combolist_item_highlight"\'><span style="padding-left:3px;" unselectable="on">Remove Style</span></td></tr>';	
...
StyleCombo.prototype.CreateInstance = function( parentToolbar )
{
	...
	opt+='<tr><td><hr/ noshade size=1></td></tr>';
	opt+='<tr><td unselectable="on" class="de_Combo_head" onclick=\'javascript:ComboOnChange("'+this.Command+'","")\' onmouseout=\'javascript:this.className="combolist_item"\' onmouseover=\'javascript:this.className="de_combolist_item_highlight"\'><span style="padding-left:3px;" unselectable="on">Remove Style</span></td></tr>';	
