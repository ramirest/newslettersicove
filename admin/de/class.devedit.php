<?php
	...
	/***********************************************************\
	|                                                            |
	|  DevEdit NX 1.8.9 Copyright Interspire Pty Ltd 2006        |
	...
	|                                                            |
	\************************************************************/
	// Define constants for calling various class functions
	...
	switch ($ToDo) {
		case "MediaManager":
		{
			...
			include($GLOBALS['dname'] . '/'. "dialog/media_manager.php");
			break;
		}
		case "DeleteImage":
		{
		...
		case "FileManager":
		{
			include($GLOBALS['dname'] . '/'. "dialog/file_manager.php");
			break;
		}
		case "InsertLink":
		{
	...
	class DevEdit
	{
		...
		function DevEdit()
		{
			...
			$this->__hideMedia = 0;
			$this->__hideCustom = 0;
			$this->__hideParagraph = 0;
			$this->__flashPath = "";
			$this->__mediaPath = "";
			$this->__hideTextBox = 0;
			$this->__hideSymbols = 0;
			$this->__disableHTMLEntities = 0;
			$this->__hideProps = 0;
			$this->__hideWord = 0;
			...
			$this->__imagePath = "";
			$this->__forcePasteWord = 0;
			$this->__forcePasteAsText = 0;
			$this->__setCursor = 0;
			$this->__hasFlashLibraries = 0;
			$this->__hasAdditionalButtons = 0;
			$this->__hasAdditionalColorButtons = 0;
			...
			$this->__hasCustomInserts = 0;
			$this->__hasCustomLinks = 0;
			$this->__hasEventListener = 0;
			$this->__snippetCSS = "";
			...
			$this->__showLoadingPleaseWait = 0;
			$this->__remotePath = "";
			$this->__startPath = "";
			$this->__startURL = "";
	...
	function iepopupstyle()
	{
		...
		function AddToolbarColorButton($ButtonName, $tooltip = "", $src = "",  $jsfunc = "", $tb = "", $tl = 1, $tp = 1)
		{
			//Add toolbar button
			$this->__hasAdditionalColorButtons = true;
			$this->__toolbarcolorbuttons[] = array("Name" => $ButtonName, "Tooltip" =>$tooltip, "ImgSrc" => $src, "JSFunc" => $jsfunc, "Toolbar" => $tb, "ToolbarLine" => $tl, "ToolbarPosition" => $tp);
		}
		...
		function __FormatToolbarColorButtons()
		{
			// Private Function - This function will return all of the toolbar buttons as JavaScript arrays
			if ($this->__hasAdditionalColorButtons == true) {
				$ciText = "[";

				for ($i = 0; $i < sizeof($this->__toolbarcolorbuttons); $i++) {
					$name = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["Name"]));
					$t = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["Tooltip"]));
					$src = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["ImgSrc"]));
					$jsfunc = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["JSFunc"]));
					$tb = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["Toolbar"]));
					$p1 = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["ToolbarLine"]));
					$p2 = str_replace("\r\n", "\\r\\n", str_replace("\"", "\\\"", $this->__toolbarcolorbuttons[$i]["ToolbarPosition"]));

					$ciText .= "[\"" . $name . "\", \"" . $t . "\", \"" . $src . "\", \"" . $jsfunc . "\", \"" . $tb . "\", " . $p1 . ", " . $p2 . "],";
				}
		...
		{
			// Hide the paragraph button
			$this->__hideParagraph = true;
			$this->__numBottomHidden++;
		}
		...
		function DisableHTMLEntities()
		{
			$this->__disableHTMLEntities = true;
		}

		function DisableComboStyles()
		{
			$this->__comboStyles = 0;
		}
		...
		function DisableMediaDeleting()
		{
 		...
 		function DisableSmartHistory()
		{
			$this->__useSmartHistory = 0;
		}
		...
		function ShowControl($Width, $Height, $ImagePath)
		{
			...
			if (isset($_SERVER['ORIG_PATH_TRANSLATED'])) {
				$script_name = str_replace(array('\\', '//'), '/', $_SERVER['ORIG_PATH_TRANSLATED']);
			} elseif (isset($_SERVER['PATH_TRANSLATED'])) {
				$script_name = str_replace(array('\\', '//'), '/', $_SERVER['PATH_TRANSLATED']);
			}

			$_SESSION['RequestedFile'] = $script_name;
	...
	$_SESSION["imgDir"] = $ImagePath;
	$_SESSION["wi"]	= $this->__hideWebImage;
	$_SESSION["tn"]	= $this->__imageDisplayType ;
	$_SESSION["du"]	= $this->__disableImageUploading ;
	$_SESSION["dd"] = $this->__disableImageDeleting ;
	$_SESSION["dt"] = $this->__docType ;
	$_SESSION["rp"] = $this->__remotePath ;
	$_SESSION["sp"] = $this->__startPath ;
	$_SESSION["su"] = $this->__startURL ;
	$_SESSION["ump"] = $this->__uploadScriptPath ;
	$_SESSION["scu"] = $this->__scUserId ;
	$_SESSION["lw"] = $this->__showLoadingPleaseWait ;
	$_SESSION["flashDir"] = $this->__flashPath ;
	$_SESSION["mediaDir"] = $this->__mediaPath ;

	$as = "";
	$c = $this->__controlName;
	$_SESSION["linkDir"] = $this->__linkPath;
	$_SESSION["name"] = $this->__controlName;
	$_SESSION["Save".$c] = $this->__hideSave;
	$_SESSION["Spelling".$c] = $this->__hideSpelling ;
	$_SESSION["RemoveTextFormatting".$c] =$this->__hideRemoveTextFormatting ;
	$_SESSION["FullScreen".$c] = $this->__hideFullScreen ;
	$_SESSION["Bold".$c] = $this->__hideBold ;
	$_SESSION["Underline".$c] = $this->__hideUnderline ;
	$_SESSION["Custom".$c] = $this->__hideCustom ;
	//$_SESSION["Custom"] = 1;
	$_SESSION["Italic".$c] = $this->__hideItalic ;
	$_SESSION["Strikethrough".$c] = $this->__hideStrikethrough ;
	$_SESSION["NumberList".$c] = $this->__hideNumberList ;
	$_SESSION["BulletList".$c] = $this->__hideBulletList ;
	$_SESSION["DecreaseIndent".$c] = $this->__hideDecreaseIndent ;
	$_SESSION["IncreaseIndent".$c] = $this->__hideIncreaseIndent ;
	$_SESSION["SuperScript".$c] = $this->__hideSuperScript ;
	$_SESSION["disableContextMenu".$c] = $this->__disableContextMenu ;
	$_SESSION["SubScript".$c] = $this->__hideSubScript ;
	$_SESSION["LeftAlign".$c] = $this->__hideLeftAlign ;
	$_SESSION["CenterAlign".$c] = $this->__hideCenterAlign ;
	$_SESSION["RightAlign".$c] = $this->__hideRightAlign ;
	$_SESSION["Justify".$c] = $this->__hideJustify ;
	$_SESSION["HorizontalRule".$c] = $this->__hideHorizontalRule;
	$_SESSION["Link".$c] = $this->__hideLink ;
	$_SESSION["ComboStyles".$c] = $this->__comboStyles ;
	$_SESSION["Anchor".$c] = $this->__hideAnchor ;
	$_SESSION["MailLink".$c] = $this->__hideMailLink ;
	$_SESSION["Help".$c] = $this->__hideHelp ;
	$_SESSION["Font".$c] = $this->__hideFont ;
	$_SESSION["Size".$c] = $this->__hideSize ;
	$_SESSION["Format".$c] = $this->__hideFormat ;
	$_SESSION["Copy".$c] = $this->__hideCopy ;
	$_SESSION["Cut".$c] = $this->__hideCut ;
	$_SESSION["Paste".$c] = $this->__hidePaste ;
	$_SESSION["File".$c] = $this->__hideFile ;
	$_SESSION["Media".$c] = $this->__hideMedia ;
	$_SESSION["Style".$c] = $this->__hideStyle ;
	$_SESSION["ForeColor".$c] = $this->__hideForeColor ;
	$_SESSION["BackColor".$c] = $this->__hideBackColor ;
	$_SESSION["Table".$c] = $this->__hideTable ;
	$_SESSION["Form".$c] = $this->__hideForm ;
	$_SESSION["Image".$c] = $this->__hideImage ;
	$_SESSION["Flash".$c] = $this->__hideFlash ;
	$_SESSION["Paragraph".$c] = $this->__hideParagraph ;
	$_SESSION["TextBox".$c] = $this->__hideTextBox ;
	$_SESSION["Symbols".$c] = $this->__hideSymbols ;
	$_SESSION["Props".$c] = $this->__hideProps ;
	$_SESSION["Clean".$c] = $this->__hideClean ;
	$_SESSION["Absolute".$c] = $this->__hideAbsolute ;
	$_SESSION["Guidelines".$c] = $this->__hideGuidelines ;
	$_SESSION["EditMode".$c] = $this->__disableEditMode ;
	$_SESSION["SourceMode".$c] = $this->__disableSourceMode ;
	$_SESSION["PreviewMode".$c] = $this->__disablePreviewMode ;
	$_SESSION["fontSizeList".$c] = $this->__fontSizeList ;
	$_SESSION["fontNameList".$c] = $this->__fontNameList ;
	$_SESSION["maximagewidth".$c] = $this->__imageMaxWidth ;
	$_SESSION["maximageheight".$c] = $this->__imageMaxHeight ;
	$_SESSION["maxUploadFileSize".$c] = $this->__maxUploadFileSize ;
	$_SESSION["tlbmode".$c] = $this->__hideToolbarMode ;
	$_SESSION["setCursor".$c] = $this->__setCursor ;
	$_SESSION["forcePasteWord".$c] = $this->__forcePasteWord ;
	$_SESSION["forcePasteAsText".$c] = $this->__forcePasteAsText ;
	$_SESSION["showLineNumber".$c] = $this->__showLineNumber ;
	$_SESSION["SingleLineReturn".$c] = $this->__useSingleLineReturn ;
	$_SESSION["guidelinesOnByDefault".$c] = $this->__guidelinesOnByDefault ;
	$_SESSION["useSmartHistory".$c] = $this->__useSmartHistory ;	
	$_SESSION["docType".$c] = $this->__docType ;
	$_SESSION["d1".$c] = $this->__disableHTMLEntities ;

	$_SESSION["mode".$c]= $this->__modeName;
	$_SESSION["spellLang".$c] = $this->__language;
	$_SESSION["width".$c] = $Width ;
	$_SESSION["height".$c] = $Height;
	$_SESSION["ComboStyles".$c] = $this->__comboStyles;
	$_SESSION["sizemode".$c] = "off";
	$_SESSION["oldw".$c] = 0;
	$_SESSION["oldh".$c] = 0;
	$_SESSION["sPath"] = $sPath;

	$_SESSION["loadedFile"] =  $this->__loadedFile;
	$_SESSION["popup_color_src".$c] =  $GLOBALS['DevEditPath'] . "/colormenu.php?name=" . $this->__controlName . "&sp=". $sPath;
	$_SESSION["deveditPath"] = $GLOBALS['DevEditPath_Full'];
	$_SESSION["deveditPath1"] = $GLOBALS['DevEditPath'];
	$_SESSION["ScriptName"]  = $scriptName;
	$_SESSION["serverurl"] = $GLOBALS['HTTPStr'] . '://' . @$_SERVER["HTTP_HOST"] ;
	$_SESSION["skinPath"] = "skins/" . $this->__skinName . "/";
	$_SESSION["skinPath2"] = $GLOBALS['HTTPStr'] . '://' . @$_SERVER["HTTP_HOST"] . $GLOBALS['DevEditPath'] .'/' . "skins/" . $this->__skinName . "/";
	$_SESSION["doc_root"] = $GLOBALS['HTTPStr'] . '://' . @$_SERVER["HTTP_HOST"] . $GLOBALS['DevEditPath'] . '/';
	
	$this->_as = $as;

	$_SESSION["useXHTML".$c] = $this->__enableXHTMLSupport ;
	$_SESSION["userdocroot"] = $_SERVER['DOCUMENT_ROOT'];
	$_SESSION["dt".$c] = $this->__dt;
?>
	...
	<script type="text/javascript">
		...
		if (!additionalColorButtons) var additionalColorButtons = new Array();
		...
		additionalColorButtons["<?php echo $this->__controlName; ?>"] = <?php echo $this->__FormatToolbarColorButtons(); ?>;
	...
	<!-- color menu -->
	<div id="colormenu<?php echo $this->__controlName; ?>" style="position:absolute;left:-500px;top:-500px;visibility:visible"><iframe tabindex="500" src="<?php echo $GLOBALS['DevEditPath']; ?>/empty.html" id="color_frame<?php echo $this->__controlName; ?>" frameborder="no" scrolling="no"></iframe></div>
...
<div style="position:absolute;top:-500px;left:-500px;visibility:hidden;">
	...
	<iframe id="<?php echo $this->__controlName.'level0'; ?>" src="<?php echo $GLOBALS['DevEditPath']; ?>/editor.php?name=<?php echo $this->__controlName; ?>&refresh=<?php echo rand(); ?>" width="100%" height="100%" scrolling="no" frameborder="no"></iframe></div></td></tr></table>
	<?php
		}
		...
		function AddHiddenFileOrFolder($FileOrFolder)
		{
			// Hide a file/folder in the media managers
			if(!isset($_SESSION["de_hidden"]))
				$_SESSION["de_hidden"] = array();

			if(!in_array($FileOrFolder, $_SESSION["de_hidden"]))
				$_SESSION["de_hidden"][] = $FileOrFolder;
		}
	}
?>
