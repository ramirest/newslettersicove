...
Save.prototype.execute = function(isUpdate)
{
	...
	if (E.mode=="source") {
		...
		code = E.HTMLTags[1]+removeWrap(img2embed(aed.documentElement.innerHTML))+E.HTMLTags[3];
...
//?
	...
	if (E.mode == "edit") {
		code = convert_param_tag_to_xhtml(unescape_href(code));
	}
...
SourceCommand.prototype.execute = function()
{
	...
	if (E.mode=="edit") {
		...
		var content = E._frame.documentElement.innerHTML;
		if (docType==1){ // DE_DOC_TYPE_HTML_PAGE
			var c =  content;
			if (browser.isFireFox1_5){
				add_title="";
				if (c.indexOf("<title>")==-1 && E._frame.title) {
					add_title = "<title>"+E._frame.title+"</title>";
				}
				var newhtml = c.match(HeadContents);
				c = newhtml[1]+newhtml[2].replace(/\n+/g,"\n")+add_title+newhtml[3];
			}
			c = E.HTMLTags[1] + c + E.HTMLTags[3];
			if (useXHTML==1) {
				iHTML = getXHTML(c);
			} else {
				iHTML = c;
			...
			var c = E.HTMLTags[1] + content + E.HTMLTags[3];
...
EditCommand.prototype.execute = function()
{
	...
	if (E.mode=="source") {
		...
		var addbase="";
		if (docType==1){ // DE_DOC_TYPE_HTML_PAGE
			html = html.toString().replace(headTag,'<head>'+addbase+'<link href="'+E.skinPath+'borders.css" type="text/css" rel="stylesheet">');
		} else{
			html = addbase + E.part[1]+removeStyle(html)+E.part[3];
		}
		var rr = new RegExp("&nbsp;","gi");
		html = html.replace(rr," ");
		html = html.replace(/&amp;/g,"&");
		html = deEngine.preserve(html);
		html = addBase(html);
		html = addBlock(html);
		E.changeHTML(embed2img(removeEditTag(html)));
	}

	E.mode="edit";	E.lastmode="edit";
	if (E.showborders==ON){
	...
	ToolbarSets.SwitchToolbarSet(toolbarmode);
	E.doStyles();
	if (E.toolbaritems.GetItem('Styles')){
...
PreviewCommand.prototype.execute = function()
{
	...
	if (E.showborders==ON) {
		E.commands.getCommand("Showborders").reset();
	}
	E.commands.getCommand("Paragraph").hideChar();
	...
	if (E.mode=="source"){
		...
		if (docType!=1){
			...
			c = E.part[1] + bodyhtml + E.part[3];
		}
		c = img2embed22(c);
	} else {
		var c = "<html>" + E._frame.documentElement.innerHTML+"</html>";
		c = img2embed22(img2embed2(c));
		c = removeBlock(c);
	}

	c = c.replace('<link href="'+E.skinPath+'borders.css" type="text/css" rel="stylesheet">','');
	E.lastmode="preview";
	E.changeHTML(c, Ifr.contentDocument,"preview");
	checkImgLoaded(Ifr.contentDocument);
	ToolbarSets.SwitchToolbarSet("_preview");
	// disable showborders style
	if(Ifr.contentDocument.getElementById(E.InstanceName+"bs"))Ifr.contentDocument.getElementById(E.InstanceName+"bs").disabled = true;
};
...
FullscreenCommand.prototype.changemode = function(str2, par, val) {
	var str = str2;
...
function Resize_DE_Editor(){
	...
	e.style.width = w+"px";
	e.style.height = h+"px";
	document.getElementById("de_body").style.height=h+"px";
}
...
FullscreenCommand.prototype.execute = function(){
	...
	if (E.fullscreen==OFF) {
		parent.addEventListener( 'resize', Resize_DE_Editor, true ) ;
		...
		var bs = parent.document.body.style;
		...
		parent.removeEventListener( "resize", Resize_DE_Editor, true ) ;
		e1.style.height = "100%";
		e.style.position = '';
		e.style.left = 0;
		e.style.top = 0;
		...
		document.getElementById("de_body").style.height=(E.oldh-5)+"px";
		E.neww = E.oldw;
		E.newh = E.oldh;
		E.fullscreen=OFF;
...
SaveStyle = function(o)
{
	...
	var sInlineStyle = o.getAttribute( 'style' ) ;
...
RestoreStyle = function( o, savedStyles )
{
	...
	if ( savedStyles.Inline )
		o.setAttribute( 'style', savedStyles.Inline, 0 ) ;
	else
...
{
	if (buffer[E.mode].devhistory.position >= buffer[E.mode].devhistory.data.length-1 ) {
		return DISABLED;
	} else {
		return OFF;
	}
...
{
	var sel = E.GetSelectedElement();
	if (!sel) {
		sel = E.parentNode("TABLE");
	}
	if (!sel) {
		return;
	}
...
URBuffer.prototype.saveHistory = function(kk)
{
	...
	try{
		...
		p = range.commonAncestorContainer.parentNode;
		if (p && p.tagName && p.tagName.toLowerCase()=="li" && kk && kk==13)p=p.parentNode;
		if (p && p.tagName && p.tagName.toLowerCase()=="html"){
			p = E._frame.body;
		}
		...
		if (p.tagName && p.tagName.toLowerCase()=="body"){
			...
			this.devhistory.offset[recpos][0] = 0;
		} else {
			this.devhistory.offset[recpos] = new Array();
			var cnt=0;
...
Editor.prototype.w = function(data)
{
	if (parent.document.getElementById(name+"_html2").value!="" && !data) {
		...
		if (html.toLowerCase().indexOf("<html")>=0) {
			var add_to_head = '<style id="'+this.InstanceName+'bs">'+border_style+'<\/style>';//'<link href="'+this.skinPath+'borders.css" type="text/css" rel="stylesheet" />';
			if (this.snippetCSS) {
		...
		if (h.toLowerCase().indexOf("<html")>=0){
			...
			html = "<html><head>";
			html += '<style id="'+this.InstanceName+'bs">'+border_style+'<\/style>';//'<link href="'+this.skinPath+'borders.css" type="text/css" rel="stylesheet" />';
...
Editor.prototype.changeHTML = function(html, frameobj, pr)
{
	...
	html = trim_space_from_style(html);
...
Editor.prototype.setEvent = function()
{
	try {
		...
		this._frame.addEventListener( 'draggesture', this.DragStart, true ) ;
		this._frame.addEventListener( 'DOMAttrModified', this.DOMSubtreeModified, true ) ;
		...
		document.addEventListener( 'blur', this.updateValue, false ) ;
	} catch(e){}
	...
	if (e){
		e.addEventListener( 'submit', this.updateValue, true ) ;
	}
...
var lostedImg = null;
Editor.prototype.DragStart = function(e)
{
	lostedImg = E._frame.getElementsByTagName("img");
};
...
{ 
	...
	htmlstr+= '<table width="100%" height="100%" class="de_container" border="0" cellspacing="0" cellpadding="0" >';
	htmlstr+= '<tr><td>';
	...
	htmlstr+= '<tr><td width="100%" height="100%" >';
		...
		htmlstr+= '<tr><td valign=top height="100%" id="leftPanel">';
		htmlstr+= '<img src="images/1x1.gif" width="1px" height="5px">&nbsp;</td>';
		htmlstr+= '<td class="de_editcontainer_in" width="100%" height="100%">';
		htmlstr+= '<iframe onblur="updateValue();" width="100%" height="1px" class="de_editcontainer" id="' + this.InstanceName + '_frame'+'"'+' onLoad="setEvent()" frameborder="no"></iframe>';
		htmlstr+= '<iframe id="' + this.InstanceName + '_preview'+'" width="100%" height="100%" scrolling="yes"  frameborder="0" class="de_editcontainer" style="display:none;"></iframe>';
	...
	htmlstr+= '<tr '+(hideTagBar?'style="display:none;"':'')+'><td class="de_TB_Toolbar" class="pathtext">';
		htmlstr+= '<table width="100%" border="0" cellspacing="0" cellpadding="0">';
		htmlstr+= '<tr><td>';
		htmlstr+= '<span id="ePath"></span>';
		htmlstr+= '</td>';
		htmlstr+= '<td>';
		htmlstr+= '<span style="float:right" class="de_pathcontainer" id="eRemoveTag"></span>';
		htmlstr+= '</td></tr>';
		htmlstr+= '</table>';
	htmlstr+= '</td></tr>';

	if (!(hideitems["EditMode"]==1 && hideitems["SourceMode"]==1 && hideitems["PreviewMode"]==1)){
		htmlstr+= '<tr><td class="de_TB_Toolbar">';
		...
		htmlstr+= '<td width="100%" valign=top>';
			...
			htmlstr+= '<div class="de_sheetline1" width="100%" height="1px"></div>';
			htmlstr+= '</td></tr>';
			htmlstr+= '<tr><td width="100%">';
			htmlstr+= '<div class="de_sheetline2" width="100%" height="1px"></div>';
			htmlstr+= '</td></tr>';
	...
	try{
		...
		this._frame.open();
		html = this.checkBODY(html);
		this._frame.write(deEngine.preserve(embed2img(addWrap(html))));
		this._frame.close();
	} catch(e) {}
...
Editor.prototype.Editable = function(e,e1)
{
	...
	if (e && (e.type=="mousedown"|| e.type=="mouseup") && e.target && e.target.tagName && e.target.tagName.toUpperCase()=="HTML" && E.mode=="edit"){
		return;
	}

	try{
		oContainer = E.getSelectedElement();
		if (!oContainer) {
			if (e1) {
...
Editor.prototype.MouseDown = function(evt)
{
	...
	if (e.target && e.target.tagName && e.target.tagName.toLowerCase() =="div" && e.target.innerHTML==""){
		...
		var sel = E._window.getSelection();
		var range = E._frame.createRange();
	...
	// if click on image
	if (e.target && e.target.tagName.toLowerCase()=="img" && e.target.id){
		if (e.target.id.indexOf("de_add")>=0){
			addblock(e.target, e);
			return false;
		} else if (e.target.id.indexOf("de_del")>=0){
			delblock(e.target, e);
			return false;
		}	
	}
...
Editor.prototype.ContextMenu = function(evt)
{
	...
	if (!evt.ctrlKey){
		...
		var node2 = null;
		for (var i=0;i<10;i++){
			node = E._frame.getElementById("de_wrap_div"+(i+1));
...
function fixedImg(){
	try{
	var newImg = E._frame.getElementsByTagName("img");
	for (var i = 0; i < lostedImg.length;i++){
		newImg[i].src = lostedImg[i].src;
	}
	} catch(e){}
	lostedImg = null;
}
...
Editor.prototype.DOMSubtreeModified = function(e)
{
	if (lostedImg)fixedImg();
}

_start_typing_node=null;
_start_typing_offset=null;
...
Editor.prototype.Redraw = function(e)
{
	...
	if (e.ctrlKey){
		...
		for(var i=0; i < ShortCutKeys.length; i++){
			...
			if (f) {
				if (E.commands.getCommand(ShortCutKeys[i][1]).getMode()!==DISABLED){
	...
	if ((e.which && (e.which>32 && e.which<256) && !e.altKey && !e.ctrlKey && !sKey[e.which]) && 
		(buffer[E.mode].devhistory.position==0 || E._window.getSelection().getRangeAt(0).startContainer!==this.lastContainer || ( E._window.getSelection().getRangeAt(0).startOffset!==this.lastOffset && E._window.getSelection().getRangeAt(0).startOffset!==this.lastOffset+1))){

		if (E._window.getSelection().getRangeAt(0).startContainer!==this.lastContainer){
			try{
				this.lastContainer = this.lastContainer.childNodes[this.lastOffset];
			} catch(y){}
		...
		this.lastContainer = E._window.getSelection().getRangeAt(0).startContainer;
		this.lastOffset = E._window.getSelection().getRangeAt(0).startOffset;
	...
	if (!e.ctrlKey && e.which != 90 && e.which != 89) {
		...
		{
			buffer[E.mode].saveHistory(e.which);
		}	
	}
...
Editor.prototype._insertBefore = function(text)
{
	...
	_end_node = sel.getRangeAt(0).startContainer
	_end_offset = sel.getRangeAt(0).startOffset;
	...
	range.setStart(_start_typing_node, _start_typing_offset);
	range.setEnd(_end_node, _end_offset);
	sel.removeAllRanges();
	sel.addRange(range);
	
	addText = aew.getSelection().toString();
	addText = addText.replace(/([\/\n\f\r])/gi,"");
	addText = addText.replace(/</gi,"&lt;");
	addText = addText.replace(/>/gi,"&gt;");
	
	text1 = addText+"&gt;";
	text1 = colourCode(text1);
	insertNodeAtSelection(document.getElementById(window.aID),text1);
	
	text = colourCode(text);
	insertNodeAtSelection(document.getElementById(window.aID),text,true);

	// set cursor to the end of first FONT tag
	sc = aew.getSelection().getRangeAt(0).startContainer;
	sc_off = aew.getSelection().getRangeAt(0).startOffset;
	aew.getSelection().removeAllRanges();
	var range2 = aed.createRange();
	range2.selectNodeContents(sc.childNodes[sc_off-1]);
	range2.collapse(false);
	aew.getSelection().addRange(range2);
	// ---
...
function insertNodeAtSelection (win, html, before,shift)
{
	...
	if (sel.anchorNode && sel.anchorNode.tagName && sel.anchorNode.tagName.toLowerCase()=="html"){
		sel.getRangeAt(0).selectNodeContents(E._frame.body);
		sel.collapseToStart();
	}
	//  end
...
{
	e = parent.document.getElementById(E.InstanceName+"main");
	e.style.width = E.neww;
	e.style.height = E.newh;
};
...
// preserve PHP tag
deEngine.addRule("(<\\?[\\s\\S]*?\\?>)");

// trim all space from STYLE defenition
// browser removes style contents if more than 1 space include (eg. style="color:    red;")
function trim_space_from_style(code){
	r = /(style=["'][^"']*?["'])/gi;
...
// unescape unicode char inside href
function unescape_href(code) { 
