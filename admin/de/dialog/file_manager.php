				...
				<body style="background-color: #EAE8E4" onLoad="Tab(<?php echo $tab; ?>);loadCustomLink();">
					...
					<div id="linkBox" class="fBox" style="display:none">
						...
						<div class="LinkBox">
							<div>
								Link Location:<br>
								<input id="link" type="text" style="font-family: Arial; font-size: 11px; width: 100%" onBlur="if (document.getElementById('link').value.substr(0,4).toLowerCase()=='www.') document.getElementById('link').value='http://'+document.getElementById('link').value;">
							</div>
							<div id="divCustomLink" style="margin-top: 5px">
								Pre-defined Links:<br>
								  <select id="libraryText" class="de_Text150" onChange="link.value = libraryText[libraryText.selectedIndex].value; target.value = libraryText[libraryText.selectedIndex].target; link.focus()">
									<option value=""></option>
								</select>
							</div>
					...
					<script>
						...
						function insertLink()
						{
							...
							window.opener.E.commands.getCommand("Save").execute("no");
						}
