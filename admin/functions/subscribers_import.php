...
*
* @version     $Id: subscribers_import.php,v 1.45 2007/05/15 07:03:55 rodney Exp $
...
class Subscribers_Import extends Subscribers
{
	...
	* The default for the "record separator" field.
	*
	* @see ImportSubscribers_Step2
	*
	* @var String
	*/
	var $DefaultRecordSeparator = 'NEWLINE';
	...
	function Process($action=null)
	{
		...
		switch ($action) {
			...
			case 'step3':
				...
				$importinfo['RecordSeparator'] = $_POST['recordseparator'];
			...
			case 'import':
				...
				switch (strtolower($importinfo['Type'])) {
					case 'file':
							...
							$subscriber_info = $this->FileGetLine($filename, true);
				...
				<script language="javascript">
					window.opener.document.location = 'index.php?Page=Subscribers&Action=Import&SubAction=PrintReport';
					window.opener.focus();
					window.close();
				</script>
				<?php
			...
			case 'printreport':
				...
				$db = &GetDatabase();
				$db->OptimizeTable(SENDSTUDIO_TABLEPREFIX . "list_subscribers");
			break;
		...
		foreach (array('duplicates', 'unsubscribes', 'bans', 'bads', 'failures') as $pos => $key) {
			...
			if ($amount > 0) {
				if ($amount == 1) {
					...
					$report .= sprintf(GetLang('ImportSubscribers_' . $key . '_Many_Link'), $key,  $this->FormatNumber($amount));
				}
			} else {
	...
	*/
	function HandleImportFile()
	{
		...
		while (true) {
			$newfilename = 'import_' . md5(uniqid(rand(), true) . SENDSTUDIO_LICENSEKEY);
		...
		$topline = $this->FileGetLine($newfilename, false);
		if (!$topline) {
			$this->ImportSubscribers_Step2(GetLang('FileNotUploadedSuccessfully'));
			return false;
		}
		...
		$fp = fopen(TEMP_DIRECTORY . '/' . $newfilename, 'r');
		while (!feof($fp)) {
			$line = fgets($fp, 10240);
		...
		if ($importinfo['Headers']) {
			// if there are headers, don't include them here. This will serve as a 'guestimate' anyway.
	...
	*/
	function LinkFields()
	{
		...
		foreach ($importlist as $pos => $option) {
			$GLOBALS['FieldName'] = sprintf(GetLang('MatchOption'), $option);
			$GLOBALS['OptionName'] = 'LinkField[' . $linkfield . ']';
			$optionlist = '';
			foreach ($field_list as $val => $opt) {
				$optionlist .= '<option value="' . $val . '">' . $opt . '</option>';
			}
	...
	*/
	function ImportSubscribers_Step2($msg=false)
	{
		...
		$GLOBALS['recordseparator'] = $this->DefaultRecordSeparator;
		...
		if ($msg) {
			...
			if (isset($_POST['RecordSeparator'])) {
				$GLOBALS['recordseparator'] = $_POST['RecordSeparator'];
			}
		}
	...
	*/
	function FileGetLine($filename=false, $wholefile=true)
	{
		...
		$fp = fopen(TEMP_DIRECTORY . '/' . $filename, 'r');
		if (!$fp) {
			return false;
		}
		...
		$length = filesize(TEMP_DIRECTORY . '/' . $filename) + 1;
		...
		if ($subscribercheck) {
			...
			if (!$unsubscribe_check) {
				if ($importinfo['Overwrite']) {
					...
					foreach ($customfields as $fieldid => $fielddata) {
						$SubscriberApi->SaveSubscriberCustomField($subscribercheck, $fieldid, $fielddata);
					}
			...
			if (!$subscriberid) {
				$importresults['failure']++;
				$importresults['failedemails'][] = $email;
				break;
			}

			foreach ($customfields as $fieldid => $fielddata) {
				$SubscriberApi->SaveSubscriberCustomField($subscriberid, $fieldid, $fielddata);
			}
	...
	* @see Session::Get
	*
	* @return Void Doesn't return anything, simply prints out the report as needed based on the session info.
	*/
	function PrintStatusReport()
	{
		...
		$GLOBALS['Report'] = $report;
		$this->ParseTemplate('Subscribers_Import_ReportProgress');
	}
