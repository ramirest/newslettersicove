...
*
* @version     $Id: customfields.php,v 1.33 2007/05/15 07:03:56 rodney Exp $
...
class CustomFields_API extends API
{
	...
	*/
	function Save()
	{
		...
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "customfields SET name='" . $this->Db->Quote($this->Settings['FieldName']) . "', defaultvalue='" . $this->Db->Quote($this->Settings['DefaultValue']) . "', required='" . $required . "', fieldsettings='" . $this->Db->Quote($settings) . "' WHERE fieldid='" . (int)$this->fieldid . "'";
	...
	/**
	* GetCustomFields
	* Gets custom field info based on the info passed in. This includes fieldid, name, fieldtype, defaultvalue, required status, other settings, when it was created and who created the field.
	*
	...
	* @param Int $perpage How many records to return. Used with start, this controls paging.
	*
	* @see DefaultOrder
	...
	*/
	function GetCustomFields($ownerid=0, $sortinfo=array(), $countonly=false, $start=0, $perpage=10)
	{
		...
		if ($sortinfo['Secondary'] && $sortinfo['SecondaryDirection']) {
