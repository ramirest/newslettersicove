...
class Email_API
{
	...
	*/
	function _SetupHeaders()
	{
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; assembling headers' . "\n", 3, $this->LogFile);
		...
		$this->_EmbeddedImages = $this->_GetImages();
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; html_headers: ' . $html_headers . "\n", 3, $this->LogFile);
			error_log('Line ' . __LINE__ . '; text_headers: ' . $text_headers . "\n", 3, $this->LogFile);
			error_log('Line ' . __LINE__ . '; multipart_headers: ' . $multipart_headers . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
	...
	*/
	function _Send_Email(&$rcpt_to, &$to, &$subject, &$body, &$headers)
	{
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; rcpt_to: ' . $rcpt_to . '; to: ' . $to . '; subject: ' . $subject . '; headers: ' . $headers . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		if ($this->SMTPServer) {
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; sending through smtp server' . "\n", 3, $this->LogFile);
			}
			if ($this->Debug) {
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; sending through php mail' . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		*/
		...
		if ($this->safe_mode || !$this->BounceAddress) {
			...
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; no bounce address or safe mode is on' . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
			...
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; bounce address set to ' . $this->_sendmailparameters . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}

			$mail_result = mail($to, $subject, $body, $headers, $this->_sendmailparameters);
		}

		if (!$mail_result) {
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; Mail broken' . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
			...
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; Mail queued' . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
	...
	*/
	function _Send_SMTP(&$rcpt_to, &$to, &$subject, &$body, &$headers)
	{
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; Connection is ' . gettype($connection) . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		if (!$connection) {
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; No connection' . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; Trying to put ' . $data . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		if (!$this->_Put_Smtp_Connection($data)) {
			...
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; Got error ' . $this->Error . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; Got response ' . $response . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; Mail accepted ' . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
	...
	*/
	function _Get_Smtp_Connection()
	{
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; smtp details: server: ' . $server . '; username: ' . $username . '; password: ' . $password . '; port: ' . $port . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
		...
		if ($responsecode == '501') {
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; Got responsecode ' . $responsecode . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
		...
		foreach ($responses as $line) {
			if ($this->Debug) {
				error_log('Line ' . __LINE__ . '; checking line ' . $line . "\n", 3, $this->LogFile);
				if ($this->memory_limit) {
					error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
				}
			}
		...
		if ($this->Debug) {
			error_log('Line ' . __LINE__ . '; require authentication: ' . (int)$requireauth . "\n", 3, $this->LogFile);
			if ($this->memory_limit) {
				error_log(basename(__FILE__) . "\t" . __LINE__ . "\t" . __FUNCTION__ . "\t" . number_format((memory_get_usage()/1024), 5) . "\n", 3, $this->MemoryLogFile);
			}
		}
	...
	*/
	function _SaveImages()
	{
		...
		foreach ($this->_EmbeddedImages as $md5 => $imageurl) {
			list($img_contents, $status) = $this->_GetImage($imageurl);
