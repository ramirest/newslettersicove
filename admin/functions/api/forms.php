...
* The Forms API.
*
* @version     $Id: forms.php,v 1.43 2007/06/04 08:13:21 chris Exp $
...
class Forms_API extends API
{
	...
	*/
	function Save()
	{
		...
		$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "forms SET name='" . $this->Db->Quote($this->name) . "', design='" . $this->Db->Quote($this->design) . "', chooseformat='" . $this->Db->Quote($this->chooseformat) . "', changeformat='" . $this->Db->Quote($this->changeformat) . "', sendthanks='" . $this->Db->Quote($this->sendthanks) . "', requireconfirm='" . $this->Db->Quote($this->requireconfirm) . "', contactform='" . $contactform . "', usecaptcha='" . $this->Db->Quote($this->usecaptcha) . "', formtype='" . $this->Db->Quote($this->formtype) . "', formhtml='" . $this->Db->Quote($this->formhtml) . "' WHERE formid='" . $this->Db->Quote($this->formid) . "'";
	...
	*/
	function GetForms($ownerid=0, $sortinfo=array(), $countonly=false, $start=0, $perpage=0)
	{
		...
		$query = "SELECT * FROM " . SENDSTUDIO_TABLEPREFIX . "forms";
		if ($ownerid) {
			$query .= " WHERE ownerid='" . $this->Db->Quote($ownerid) . "'";
		}
	...
	*/
	function GetFormDesigns()
	{
		...
		foreach ($designs as $name => $type) {
			if (strtolower($name) == 'cvs' || strtolower($name) == 'captcha') {
				continue;
			}
	...
	*
	* @param $inside_sendstudio Boolean Pass in whether we are viewing the form from inside the application or not. This allows us to include/exclude information accordingly. This stops a problem where viewing a form will log you out of the admin control panel.
	*
	* @see GetFormDesign
	* @see FetchFile
	* @see formtype
