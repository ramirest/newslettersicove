...
*
* @version     $Id: customfields_date.php,v 1.10 2007/03/29 08:22:20 chris Exp $
...
class CustomFields_Date_API extends CustomFields_API
{
	...
	*/
	function CheckData($data=array())
	{
		if (!is_array($data)) {
			...
			foreach ($this->Options['Key'] as $p => $type) {
	...
	*/
	function DisplayFieldOptions($chosen=array(), $useoptions=false, $formid=0)
	{
		...
		$daylist = '<select name="CustomFields['.$this->fieldid.'][dd]">';
		...
		$monthlist = '<select name="CustomFields['.$this->fieldid.'][mm]">';
		for ($i=1; $i<=12; $i++) {
			$monthlist.='<option value="'.sprintf("%02d",$i).'">'.GetLang($this->Months[$i]) . '</option>';
		}
		$monthlist.='</select>';

		$yearlist = '<select name="CustomFields['.$this->fieldid.'][yy]">';
