...
*
* @version     $Id: templates.php,v 1.41 2007/05/28 07:04:49 scott Exp $
...
class Templates extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		$secondary_actions = array('view', 'save', 'complete', 'step1', 'addtemplate', 'builtin', '', 'activate', 'deactivate', 'activateglobal', 'deactivateglobal');
		if (in_array($action, $secondary_actions)) {
			$access = $user->HasAccess('Templates');
		} else {
	...
	*/
	function ManageTemplates()
	{
		...
		if ($user->TemplateAdmin()) {
			...
			$GLOBALS['CreateTemplatePreview'] = GetLang('CreateTemplatePreview');
		}
	...
	*/
	function ManageBuiltInTemplates()
	{
		...
		$template_row = $single_row = $this->ParseTemplate('Templates_BuiltIn_Manage_Row', true, false);
		...
		if (!empty($template_names)) {
			foreach ($template_names as $p => $name) {
				...
				$templatecell = $this->ParseTemplate('Templates_BuiltIn_Manage_Cell', true, false);

				$single_row = str_replace('%%TPL_Template_Cell_' . $c . '%%', $templatecell, $single_row);
				if (($c % 4) == 0) {
					$template_display .= $single_row;
					$single_row = $template_row;
					$c = 0;
				}
			}
		}
		...
		if (!empty($template_packs)) {
			...
			while ($c < 4) {
				$c++;
				$single_row = str_replace('%%TPL_Template_Cell_' . $c . '%%', '', $single_row);
			}
			$template_display .= $single_row;
		}
