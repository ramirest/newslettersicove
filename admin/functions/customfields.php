...
*
* @version     $Id: customfields.php,v 1.39 2007/05/15 07:03:55 rodney Exp $
...
class CustomFields extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		if ($action == 'associate') {
			$access = $user->HasAccess('customfields');
		} else {
			$access = $user->HasAccess('customfields', $action);
		}
	...
	*/
	function EditCustomField($fieldid=0)
	{
		...
		if (in_array($fieldapi->fieldtype, $this->MoreOptions)) {
			$extralist_template = 'CustomField_Form_Step2_' . $fieldapi->fieldtype . '_list';
	...
	*/
	function CreateCustomField_Step1()
	{
		...
		foreach ($this->CustomFieldTypes as $p => $type) {
			$typelist .= '<option value="' . $type . '">' . GetLang('CustomFieldType_' . strtolower($type)) . '</option>';
		}
	...
	*/
	function ManageCustomField_Lists($fieldid=0, $newfield=false)
	{
		...
		foreach ($lists as $listid => $listdetails) {
			...
			$GLOBALS['listname'] = htmlspecialchars($listdetails['name'], ENT_QUOTES, SENDSTUDIO_CHARSET);
			$list_assoc .= $this->ParseTemplate('Customfield_Form_Associate', true, false);
		}
		$GLOBALS['ListAssociations'] = $list_assoc;
	...
	*/
	function ManageCustomFields()
	{
		...
		if ($user->HasAccess('CustomFields', 'Create')) {
			...
			$GLOBALS['CustomFields_Heading'] = GetLang('Help_CustomFieldsManage_HasAccess');
		}
