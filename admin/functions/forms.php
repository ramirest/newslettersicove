...
*
* @version     $Id: forms.php,v 1.54 2007/06/20 08:13:16 chris Exp $
...
class Forms extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
		...
		$secondary_actions = array('preview', 'gethtml', 'view', 'finish', 'step2', 'step3', 'step4', 'step5', 'manage');
			...
			case 'step2':
				...
				if (in_array($session_form['FormType'], $ftypes)) {
					if (!empty($_POST['fields'])) {
						$session_form['CustomFields'] = $_POST['fields'];
					}
				}

				// in case there is a custom field on multiple lists (eg "name"), do a unique check
				$session_form['CustomFields'] = array_unique($session_form['CustomFields']);
		...
		foreach ($lists as $listid => $details) {
			...
			if ($form['FormType'] != 'u') {
				...
				foreach ($customfields as $pos => $cf_details) {
					$checked = '';
					if (in_array($cf_details['fieldid'], $form['CustomFields'])) {
						$checked = ' CHECKED';
			...
			if (empty($customfields)) {
				...
				$GLOBALS['CustomFieldHeading'] = sprintf(GetLang('ChooseCustomFields'), htmlspecialchars($details['name'], ENT_QUOTES, SENDSTUDIO_CHARSET));
