...
* @see Email_API
*
* @version     $Id: send_friend.php,v 1.9 2007/04/27 01:45:32 chris Exp $
...
$subscriber_id = $session->Get('Subscriber');
...
$autoresponder = $session->Get('Autoresponder');
...
$emailtext = str_replace('%REFERRER_EMAIL%', $fromemail, $emailtext);
if ($textbody) {
	$textbody = $introduction . "\n" . $emailtext . "\n" . $textbody;
}

$emailhtml = $formapi->GetPage('SendFriendPage', 'emailhtml');
$emailhtml = str_replace('%REFERRER_EMAIL%', $fromemail, $emailhtml);
if ($htmlbody) {
...
*/
$forwardip = $statsapi->GetRealIp();
$forwardtime = $statsapi->GetServerTime();

$statid = $session->Get('Statid');
